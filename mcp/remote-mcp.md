...
⚠️ **Heads up — terminology has changed.** ChatGPT recently renamed **Connectors → Apps**. The same MCP feature lives under **Settings → Apps** now. The in-chat menu still says _Connectors_ though, so don't be confused.
...
You may see a notice that _"CIMD is unavailable because the server did not advertise CIMD support"_ — that is expected. OpenAlgo advertises DCR, not Client Identifier Metadata Document. DCR is the right choice.
...
> ⚠️ _Custom MCP servers introduce risk. OpenAI hasn't reviewed this MCP server. Attackers may attempt to steal your data or trick the model into taking unintended actions, including destroying data._
...
> ❌ _OAuth authorization failed: unauthorized\_client_
...
4. The **Pending approvals** card shows the new client. The `client_name` will be set by ChatGPT (e.g. _"ChatGPT MCP Connector"_ or whatever you typed in **Name**), and `client_id_issued_at` will be the last few seconds
...
2. A small menu appears with two options — pick **Add custom connector**
   * (The other option, _Browse connectors_, is for first-party Anthropic ones — Gmail, Drive, etc.)
...
> _Only use connectors from developers you trust. Anthropic does not control which tools developers make available and cannot verify that they will work as intended or that they won't change._
...
You can also expand each group and override permissions on individual tools — e.g. _Always allow_ most tools but force _Ask me_ specifically for `cancel_all_orders`.
...
> _"Show me the current LTP of NIFTY 50 and give me a quick view of my open positions."_

Claude shows tool-call cards you can expand to inspect the exact parameters. If a tool is set to _Ask me_, you'll get a permission prompt with **Allow once / Always allow / Deny**.
...
* _"Place a limit BUY for 1 share of TCS at ₹3500 in CNC product on NSE"_ — `place_order`
* _"Modify my last open INFY order — change the quantity to 5"_ — `modify_order`
* _"Cancel all my open orders"_ — `cancel_all_orders`
* _"What was my P\&L today?"_ — `get_tradebook` + summary

For more example prompts per tool, see Tool References — every prompt there works on Remote MCP too.
...
If the AI says _"the tool returned an error"_ and refuses to elaborate, check the audit log — `outcome=error` rows include the reason.
...
| _"Server doesn't implement OAuth"_ | First connect                   | Old OpenAlgo build                         | Update to 2.0.1.0+                                                  |
| _"CIMD is unavailable"_            | ChatGPT advanced settings       | OpenAlgo advertises DCR, not CIMD          | Expected — pick **DCR**                                             |
...
| Section                 | Use                                                                                                                                                                                                                |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Pending approvals**   | New DCR clients land here. Approve only ones you recognize (the client\_name is set by the hosted client itself, e.g. _"ChatGPT MCP Connector"_).                                                                  |
...
* **Per-page CSP** — the consent screen sets a tight CSP that allows form submission only to the registered redirect\_uri's origin
